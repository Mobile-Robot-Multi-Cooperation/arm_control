...
using ControllerReferenceMsg = arm_controller::ARMController::ControllerReferenceMsg;
...
{
ARMController::ARMController() : controller_interface::ControllerInterface(), rt_location_ptr_(nullptr){}
...
controller_interface::CallbackReturn ARMController::on_init()
{
  ...
  try
  {
    param_listener_ = std::make_shared<arm_controller::ParamListener>(get_node());
  }
  ...
  // 手臂接口定义
  armInterface_ = std::make_shared<arm_interface::ARMInterface>(taskFile, libFold, urdfFile);
  // 三个节点
  ...
  //use for debug
  cmd_publisher_ = get_node()->create_publisher<geometry_msgs::msg::Twist>("arm/cmd_vel", 10);
...
// 意义不大，固定的，可以改成末端的tf
void ARMController::tf_callback() {
        std::lock_guard<std::mutex> lock(transform_mutex_);
        try {
            auto transform_stamped = tf_buffer_->lookupTransform("world", "elfin_base_link", tf2::TimePointZero);
            ...
            RCLCPP_WARN(get_node()->get_logger(), "Could not transform 'world' to 'elfin_base_link': %s", ex.what());
        }
}

void ARMController::setupMpc()
{
  mpc_ = std::make_shared<ocs2::GaussNewtonDDP_MPC>(armInterface_->mpcSettings(),armInterface_->ddpSettings(),armInterface_->getRollout(),
                                            armInterface_->getOptimalControlProblem(),armInterface_->getInitializer());
  auto rosReferenceManagerPtr = std::make_shared<ocs2::RosReferenceManager>(robot_name_, armInterface_->getReferenceManagerPtr());
...
void ARMController::setupMrt()
{
  mrtRosInterface_ = std::make_shared<ocs2::MRT_ROS_Interface>(robot_name_);
  mrtRosInterface_->initRollout(&(armInterface_->getRollout()));
}
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  ref_subscriber_ = get_node()->create_subscription<ControllerReferenceMsg>(
    ...
    std::bind(&ARMController::reference_callback, this, std::placeholders::_1));
...
void ARMController::reference_callback(const std::shared_ptr<ControllerReferenceMsg> msg)
{
...
ocs2::TargetTrajectories ARMController::goalPoseToTargetTrajectories(const Eigen::Vector3d& position, const Eigen::Quaterniond& orientation,
  ...
  const rclcpp_lifecycle::State & previous_state)
{
  ...
  if(use_gui_target)
  {
    auto bound_function = std::bind(&ARMController::goalPoseToTargetTrajectories, this, std::placeholders::_1, std::placeholders::_2,std::placeholders::_3);
    targetPoseCommandInterface_ = std::make_shared<arm_interface::ARMJoyTargetTrajectories>(target_node_,robot_name_,bound_function,task_file_);
...
// 获取机械臂的状态
void ARMController::updateStateEstimation(const rclcpp::Time time,const rclcpp::Duration &period)
{
  std::lock_guard<std::mutex> lock(transform_mutex_);
  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
  ...
  if(first_restart == false)
  {
    ...
    const ocs2::vector_t zeroInput =
        ocs2::vector_t::Zero(armInterface_->getARMModuelInfo().inputDim);
  ...
  if(mpc_node_well==false){
    ...
    if(!mrtRosInterface_->initialPolicyReceived() && rclcpp::ok()){
      ...
      // while (!mrtRosInterface_->initialPolicyReceived() && rclcpp::ok()) {
      // RCLCPP_INFO_STREAM(get_node()->get_logger(), "Initial policy has been receiveda.");
      // }
    ...
    RCLCPP_INFO_STREAM(get_node()->get_logger(), "Initial policy has been receivedb.");
  ...
  const ocs2::scalar_t dt = 1.0/armInterface_->mpcSettings().mrtDesiredFrequency_;
