...
#include "moying_arm_hardware/arm_hardware_interface.hpp"
  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  arm_handle_ = std::make_shared<moying_arm_hardware::ArmEthercatDriver>(ecat_manager_,arm_ethercat_node,arm_config);
  ...
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  ...
  for(auto & ah:arm_handle_->module_infos)
  {
    ...
    ah.axis2.velocity_cmd = ah.axis2.position;
  }
...
PLUGINLIB_EXPORT_CLASS(
  moying_arm_hardware::ArmHardwareInterface, hardware_interface::SystemInterface)
