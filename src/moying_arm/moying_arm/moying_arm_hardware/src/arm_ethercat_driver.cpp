#include "moying_arm_hardware/arm_ethercat_driver.hpp"
#include "hardware_interface/handle.hpp"
#include "hardware_interface/hardware_info.hpp"
#include "hardware_interface/system_interface.hpp"
#include "hardware_interface/types/hardware_interface_return_values.hpp"
#include "yaml-cpp/yaml.h"
#include <baichuan_arm_dynamics_models/dynamics_models.h>
...
ArmEthercatDriver::ArmEthercatDriver(std::shared_ptr<elfin_ethercat_driver_plant::EtherCatManager> ecat_manager,
    const rclcpp::Node::SharedPtr& node,YAML::Node& config)
{
  ...
  {
    ethercat_drivers_[i] = new elfin_ethercat_driver_plant::ElfinEtherCATDriver(em,elfin_driver_names_[i],n_,config);
  }
  module_infos.clear();
                                          ...
                                          const std::vector<std::string>& stop_interfaces)
{
  ...
  for(auto& key : start_interfaces)
  {
    for(std::size_t i=0;i<module_infos.size();i++)
    {
      ...
      if(key == module_infos[i].axis2.name + "/" + "postion")
      {
...
hardware_interface::return_type ArmEthercatDriver::perform_command_mode_switch(const std::vector<std::string>&, const std::vector<std::string>&)
{
  ...
  std::vector<double> joint_position = {module_infos[0].axis1.position,module_infos[0].axis1.position,
    module_infos[1].axis1.position,module_infos[1].axis2.position,
    module_infos[2].axis1.position,module_infos[2].axis2.position};
  Eigen::VectorXd gravity =  dyn_model->gravityTerm(joint_position);  // 重力补偿
