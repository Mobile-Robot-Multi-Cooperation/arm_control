...
/******************************************************************************************************/
// 订阅目标点 发布和更新轨迹
UR5JoyTargetTrajectories::UR5JoyTargetTrajectories(
  ...
  timer_ = node_->create_wall_timer(
      100ms, std::bind(&UR5JoyTargetTrajectories::timer_callback,this));
  // std::string joy_topic_name=node->get_parameter("joy_topic_name").as_string(); 订阅手柄话题
  joy_command_subsciption_ = node_->create_subscription<sensor_msgs::msg::Joy>("/ur5/joy", rclcpp::SystemDefaultsQoS(), [this](const  sensor_msgs::msg::Joy::SharedPtr twist)
  {
  ...
  // 把机械臂调到初始位置
  RCLCPP_INFO_STREAM(node_->get_logger(),"[UR5JoyTargetTrajectory] Loading...");
  boost::filesystem::path taskFilePath(taskFile);
  if (boost::filesystem::exists(taskFilePath)) {
    RCLCPP_INFO_STREAM(node_->get_logger(),"[UR5JoyTargetTrajectory] Loading task from:"<<taskFilePath);
  } else {
    RCLCPP_ERROR_STREAM(node_->get_logger(),"[UR5JoyTargetTrajectory] Can't loading task file");
    throw std::invalid_argument("[UR5JoyTargetTrajectory] task file not found: " + taskFilePath.string());
  }
...
void UR5JoyTargetTrajectories::timer_callback() {
