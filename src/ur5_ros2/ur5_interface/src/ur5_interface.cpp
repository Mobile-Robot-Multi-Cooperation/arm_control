...
#include "ur5_interface/ur5_interface.h"

#include "ur5_interface/constraint/end_effort_constraint.h"
// #include "ur5_interface/constraint/base_constraint.h"
#include "ur5_interface/constraint/ur5_self_collision_constraint.h"
...
#include "ur5_interface/cost/quadratic_input_cost.h"
// Boost
...
UR5Interface::UR5Interface(const std::string& taskFile, const std::string& libraryFolder,
                                      const std::string& urdfFile)
{
  // 检查任务文件是否存在
  boost::filesystem::path taskFilePath(taskFile); 
  if (boost::filesystem::exists(taskFilePath)) {
    std::cerr << "[Interface] Loading task file: " << taskFilePath << std::endl;
  } else {
    throw std::invalid_argument("[ur5Interface] Task file not found: " + taskFilePath.string());
  }
  // 检查urdf文件是否存在
  boost::filesystem::path urdfFilePath(urdfFile);
  if (boost::filesystem::exists(urdfFilePath)) {
    std::cerr << "[ur5Interface] Loading Pinocchio model from: " << urdfFilePath << std::endl;
  } else {
    throw std::invalid_argument("[ur5Interface] URDF file not found: " + urdfFilePath.string());
  }
  // 检查库文件是否存在
  boost::filesystem::path libraryFolderPath(libraryFolder);
  boost::filesystem::create_directories(libraryFolderPath);
  std::cerr << "[ur5Interface] Generated library path: " << libraryFolderPath << std::endl;
  ...
  // 创建 ur5ModuleInfo 对象
  ur5ModuleInfo_ = ur5_interface::createUR5ModuleInfo(*pinocchioInterfacePtr_,baseFrame, eeFrame);
  ...
  initialState_.setZero(ur5ModuleInfo_.stateDim);
  const int baseStateDim = ur5ModuleInfo_.stateDim - ur5ModuleInfo_.armDim;
  const int armStateDim = ur5ModuleInfo_.armDim;
  ...
  problem_.stateSoftConstraintPtr->add("endEffector", getEndEffectorConstraint(*pinocchioInterfacePtr_, taskFile, "endEffector",
                                                                                usePreComputation, libraryFolder, recompileLibraries, ur5ModuleInfo_.eeFrame));
  ...
  problem_.dynamicsPtr.reset(
          new UR5Dynamics(ur5ModuleInfo_, "dynamics", libraryFolder, recompileLibraries, true));

  // 预计算，只是一个指针
  if (usePreComputation) {
    problem_.preComputationPtr.reset(new UR5PreComputation(*pinocchioInterfacePtr_, ur5ModuleInfo_));
  }
  ...
  initializerPtr_.reset(new DefaultInitializer(ur5ModuleInfo_.inputDim));
}
...
/******************************************************************************************************/
// 从task获取输入成本矩阵 R
std::unique_ptr<ocs2::StateInputCost> UR5Interface::getQuadraticInputCost(const std::string& taskFile)
{
  ocs2::matrix_t R = ocs2::matrix_t::Zero(ur5ModuleInfo_.inputDim, ur5ModuleInfo_.inputDim); // 零矩阵
  const int baseInputDim = ur5ModuleInfo_.inputDim - ur5ModuleInfo_.armDim;
  const int armStateDim = ur5ModuleInfo_.armDim ;
  ...
  return std::make_unique<ur5_interface::QuadraticInputCost>(std::move(R), ur5ModuleInfo_.stateDim);
}
...
/******************************************************************************************************/
// 增加位置和速度软约束
std::unique_ptr<ocs2::StateInputCost> UR5Interface::getJointLimitSoftConstraint(const PinocchioInterface& pinocchioInterface,
                                                                                        const std::string& taskFile)
{
  ...
  const int armStateDim = ur5ModuleInfo_.armDim;
  const int baseInputDim = ur5ModuleInfo_.inputDim - ur5ModuleInfo_.armDim;
  const int armInputDim = ur5ModuleInfo_.armDim;
  ...
  {
    // arm limits
    vector_t lowerBound = vector_t::Zero(ur5ModuleInfo_.inputDim);
    vector_t upperBound = vector_t::Zero(ur5ModuleInfo_.inputDim);
    scalar_t muVelocityLimits = 1e-2;
    scalar_t deltaVelocityLimits = 1e-3;

    // ur5 joint DOFs velocity limits
    ...
    inputLimits.reserve(ur5ModuleInfo_.inputDim);
    std::cerr << " #### =============================FOR======================================\n";
    for (int i = 0; i < ur5ModuleInfo_.inputDim; ++i) {
  ...
  boxConstraints->initializeOffset(0.0, vector_t::Zero(ur5ModuleInfo_.stateDim), vector_t::Zero(ur5ModuleInfo_.inputDim));
  return boxConstraints;
}

// 末端执行器约束，需要修改
std::unique_ptr<ocs2::StateCost> UR5Interface::getEndEffectorConstraint(const PinocchioInterface& pinocchioInterface,
                                                                                ...
                                                                                bool recompileLibraries,const std::string endEffectorName)
{
  ...
  if (usePreComputation) {
    std::cerr << " #### =============================================================================1\n";
    UR5PinocchioMapping pinocchioMapping(ur5ModuleInfo_); // 系统输入到pinocchio的映射
    ...
    UR5PinocchioMappingCppAd pinocchioMappingCppAd(ur5ModuleInfo_);
    PinocchioEndEffectorKinematicsCppAd eeKinematics(pinocchioInterface, pinocchioMappingCppAd, {endEffectorName},
                                                     ur5ModuleInfo_.stateDim, ur5ModuleInfo_.inputDim,
...
// 自碰撞约束
std::unique_ptr<ocs2::StateCost> UR5Interface::getSelfCollisionConstraint(const PinocchioInterface& pinocchioInterface,
  ...
  std::unique_ptr<StateConstraint> constraint;
  if (usePreComputation) {
    constraint = std::make_unique<UR5SelfCollisionConstraint>(UR5PinocchioMapping(ur5ModuleInfo_),
                                                                            std::move(geometryInterface), minimumDistance);
  } else {
    constraint = std::make_unique<SelfCollisionConstraintCppAd>(
        pinocchioInterface, UR5PinocchioMapping(ur5ModuleInfo_), std::move(geometryInterface), minimumDistance,
